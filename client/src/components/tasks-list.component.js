...
const Task = props => (
  <div  className='bg-light' style={{marginTop : 10 + 'px', marginBottom: 10 + 'px', paddingLeft: 10 + '%', paddingRight: 10 + '%'}}>
    ...
    {props.task.complete ? <p style={{fontSize: 20 + 'px'}}>Status: Completed</p> : <p style={{fontSize: 20 + 'px'}}>Status: Incomplete</p>}
    <button onClick={() => { props.deleteTask(props.task._id) }} className='btn btn-primary bg-success'>
					Delete
				  </button>
    {props.task.complete ? "" : <button className='btn btn-primary bg-success' style={{marginLeft: 10 + 'px'}}> <Link to={"/edit/"+props.task._id} style={{color: "white", textDecoration: 'none'}}> edit</Link> 
    </button> }
    </details>
    </div>
)
